...
void setup() {
  ...
  SD.begin(chipSelect);
  mySwitch.enableReceive(0); //Initializing the RCSwitch receiver
...
void loop() {
  if (mySwitch.available()) {
    ...
    if (receivedValue == 999999) { // Start the Data Acquisition (DAQ) process
      startDAQ = true;

      // Generate a new filename only if it hasn't been set yet
      if (currentFilename == "") {
        currentFilename = generateUniqueFilename();

        // Write the duty cycle to this new file immediately
        File dataFile = SD.open(currentFilename, FILE_WRITE);
        if (dataFile) {
          dataFile.println("Duty Cycle: " + String(duty_cycle));
          dataFile.close();
        }
      }
      Serial.println(duty_cycle); // Display the duty cycle in the serial monitor
      
    } else if (receivedValue == 888888) { // Terminate the DAQ process
      startDAQ = false;
      currentFilename = "";  // Reset filename to create a new file when logging restarts
      Serial.println("Data logging terminated.");
      
    } else { // Set a new duty cycle
      duty_cycle = intBitsToFloat(receivedValue); 
    }
    
    mySwitch.resetAvailable(); // Reset the availability of the RCSwitch after reading the data
  }

  if (!startDAQ) {
    return;  //Exit loop if startDAQ is false
  }
